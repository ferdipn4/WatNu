...
function buildPrompt(
  ...
  return [
    ...
    "",
    "Recurring events: \"elke maandag\", \"every Tuesday\", \"wekelijks\" → `recurrence`",
    '"weekly"; "om de week" / "every other week" → "biweekly"; "elke eerste vrijdag',
    'van de maand" / "monthly" → "monthly"; anything else → null. For a recurring',
    "event `start` is its next occurrence from today (the next such weekday at the",
    "stated time — that is read from the poster, not invented), and `repeat_until`",
    "is the last date only when the poster states one (\"t/m 15 december\"), else null.",
    "`recurrence` and `repeat_until` are never missing fields.",
    "",
  ...
  "events": [
    {
      ...
      "recurrence": "weekly | biweekly | monthly when the poster says it repeats, else null",
      "repeat_until": "YYYY-MM-DD when the poster states a last date, else null",
